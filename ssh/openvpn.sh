...
rm -rf /etc/openvpn
mkdir -p /etc/openvpn
wget -O /etc/openvpn/openvpn.zip "https://github.com/izulx1/project/raw/master/ssh/openvpn.zip" >/dev/null 2>&1
...
cd /var/www/html/
zip OpenVPN.zip tcp.ovpn udp.ovpn ssl.ovpn ws-ssl.ovpn > /dev/null 2>&1
cd
